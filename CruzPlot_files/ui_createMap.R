          ...
          fluidRow(
            ...
            tabBox(
                         ...
                         column(6, 
                                fluidRow(
                                  box(
                                    ...
                                    helpText(HTML("For longitude values, please use the range -180 to 180. ", 
                                                  "Thus, for a map of the Pacific, you could enter 130 and -110 for ",
                                                  "the left and right longitude, respectively", "<br/>", 
                                                  "Click 'Update plot dimensions' button to replot map", "<br/>", 
                                    ...
                                    fluidRow(
                                      column(width = 6,
                                             ...
                                             selectInput("resolution", label = h5("Resolution"), 
                         ...
                         column(6, 
                                fluidRow(
                                  box(
                                    ...
                                    conditionalPanel(condition = "input.coast==true",
                                                     helpText(paste("Map limits will automatically be updated to the extent of the", 
                                                     ...
                                                     # actionButton("coast.map", label = "Choose NGulf.coast.csv")
                                                     # actionButton("coast.map", label = h5("Set map limits to limits of coastline file"))
                                    )
                                  ), 
                                  box(
                                    title = "Scale bar", status = "warning", solidHeader = FALSE, width = 12, collapsible = TRUE,
                                    checkboxInput("bar", "Plot scale bar", value = FALSE),
                                    conditionalPanel(condition = "input.bar==true",
                                                     ...
                                                     fluidRow(
                                                       column(width = 6,
                                                              ...
                                                              radioButtons("scale.units", h5("Scale bar units"), 
                                                                           ...
                                                                           selected = 2), 
                                                              numericInput("scale.width", h5("Width of bar"), 
                       ...
                       fluidRow(
                         box(
                           ...
                           fluidRow(
                             column(7,
                                    ...
                                    fluidRow(
                                      ...
                                      column(6, 
                                             ...
                                             conditionalPanel("input.planned_transects_name_type == 1", 
                                                              uiOutput("planned_transects_names_uiOut_select")),
                                             conditionalPanel("input.planned_transects_name_type == 2", 
                                                              textInput("planned_transects_name_text", 
                                                                        NULL, value = "Primary"))
                                      )
                             ...
                             column(5, 
                                    conditionalPanel("output.cruzMapPlannedTransects_Conditional", 
                                                     checkboxInput("planned_transects_plot", "Plot planned transect lines", 
                                                                   value = FALSE),
                                                     column(12, 
                                                            conditionalPanel("input.planned_transects_plot", 
                                                                             helpText(paste("Order in which transects are selected to be", 
                                                                                            "plotted corresponds to order of specified", 
                                                                                            "color(s) and line width(s)")), 
                                                                             ui.selectize.instructions(), 
                                                                             uiOutput("planned_transects_toplot_uiOut_select"),
                                                                             fluidRow(
                                                                               column(7, 
                                                                                      selectizeInput("planned_transects_color", 
                                                                                                     label = h5("Transect color(s)"), 
                                                                                                     choices = cruz.palette.color,
                                                                                                     selected = "gray", multiple = TRUE)
                                                                               ), 
                                                                               column(5, 
                                                                                      # uiOutput("planned_transects_lw_uiOut_text")
                                                     ...
                                                     ui.new.line(), 
                                                     checkboxInput("planned_transects_remove", 
                                                                   "Select planned transect lines to remove", value = FALSE),
              ...
              tabPanel("Ticks & Labels",
                       ...
                       fluidRow(
                         conditionalPanel(condition = "input.tick==true",
                                          box(title = "Tick marks", status = "warning", solidHeader = FALSE, width = 6, collapsible = TRUE, 
                                              height = 437, 
                                              fluidRow(
                                                column(width = 6,
                                                ...
                                                column(width = 6,
                                                       ...
                                                       numericInput("tick.length", label = h5("Tick length"), 
                                                                    value = 1.0, min = 0, max = 2.5, step = 0.1)
                                                )
                                              )
                                          ),
                                          box(title = "Tick labels", status = "warning", solidHeader = FALSE, width = 6, collapsible = TRUE, 
                                              height = 437, 
                                              fluidRow(
                                                column(width = 6,
                                                       checkboxInput("tick.left.lab", label = "Left", value = TRUE),
                                                       checkboxInput("tick.bot.lab", label = "Bottom", value = TRUE),
                                                       numericInput("label.lon.start", h5("Start longitude tick labels at"), 
                                                                    value = as.character(start.tick$lon)),
                                                ...
                                                column(width = 6,
                                                       ...
                                                       numericInput("label.lat.start", h5("Start latitude tick labels at"), 
                                                                    value = as.character(start.tick$lat)),
                                                       numericInput("label.tick.size", label = h5("Tick label size"), 
              ...
              tabPanel("Map Labels", 
                       fluidRow(
                         box(
                           title = "Title", status = "warning", solidHeader = FALSE, width = 6, collapsible = TRUE, 
                           height = 315, 
                           textInput("label.title", h5("Map title"), value = ""),
                           fluidRow(
                             column(width = 6,
                                    selectInput("label.title.font", label = h5("Title font"), 
                                                choices = font.family, selected = 1)
                             ),
                             column(width = 6,
                                    numericInput("label.title.size", label = h5("Title size"), 
                                                 value = 1.5, min = 0.1, max = 3, step = 0.1)
                             )
                           )
                         ),
                         box(title = "Axis labels", status = "warning", solidHeader = FALSE, width = 6, collapsible = TRUE, 
                             height = 402, 
                             textInput("label.axis.lon", h5("Longitude axis label"), value = ""),
                             textInput("label.axis.lat", h5("Latitude axis label"), value = ""),
                             fluidRow(
                               column(width = 6,
                                      selectInput("label.axis.font", label = h5("Axis label font"), 
                                                  choices = font.family, selected = 1)
                               ),
                               column(width = 6,
                                      numericInput("label.axis.size", label = h5("Axis label size"), 
                                                   value = 1.2, min = 0.1, max = 3, step = 0.1)
                               )
                             )
                         )
                       )
                         ...
                         column(width = 6,
                                fluidRow(
                                  box(
                                    ...
                                    radioButtons("color_style", label = NULL,
                                                 choices = list("Color" = 1, "Gray scale" = 2), 
                                                 selected = 1)
                                  ),
                                  box(
                                    title = "Land", status = "warning", solidHeader = FALSE, collapsible = TRUE, width = 12, 
                                    height = 340, # height needed to disply all color selections within shaded box
                                    fluidRow(
                                      column(width = 6,
                                             checkboxInput("color_land_all", label = "Color all land", value = TRUE)
                                      ),
                                      column(width = 6,
                                             conditionalPanel(condition = "input.color_land_all==true",
                                                              selectInput("color.land", label = h5("Land color"), 
                                                                          choices = cruz.palette.color, 
                                                                          selected = "bisque1")
                                             )
                                      )
                                ...
                                fluidRow(
                                  box(
                                    ...
                                    radioButtons("color_water_style", label = h5("Ocean color style"),
                                                 ...
                                                 selected = 1),
                                    conditionalPanel(condition = "input.color_water_style==1",
                                                     selectInput("color.water", label = h5("Water color"), 
                                                                 choices = cruz.palette.color,
                                                                 selected = "white")
                                    ),
                                    conditionalPanel(condition = "input.color_water_style==2",
                                                     helpText("Options:"),
                                                     helpText("1) Load csv file with 3 columns: latitude, longitude, and depth"),
                                                     helpText(HTML(paste("2) Download bathymetric data from NOAA website", 
                                                                         "based on lat/lon of map.",  "<br/>", 
                                                     ...
                                                     conditionalPanel(condition = "input.depth_style==2",
                                                                      fileInput("depth.file", h5("Bathymetric csv file"), accept = '.csv'),
                                                                      checkboxInput("depth.header", label = h5("Header"), value = TRUE),
                                                                      fluidRow(
                                                                        column(width = 6,
                                                                               radioButtons("depth.sep", label = h5("Separator"),
                                                                                            c(Comma = ",", Semicolon = ";", Tab = "\t"), ",")
                                                                        ),
                                                                        column(width = 6,
                                                                               radioButtons("depth.quote", label = h5("Quote"),
                                                                                            choices = list("None" = "", "Double quote" = '"', 
                                                                                                           "Single quote" = "'"), 
                                                                                            selected = '"')
                                                                        )
                                                                      )
                                                     ),
                                                     conditionalPanel(condition = "input.depth_style==1",
                                                                      textInput("depth.res", 
                                                                                h5("Bathymetric data resolution, in minutes (range: 0-60)"), 
                                                                                value = "10")
                                                     )
                                    )
                                  )
              ...
              tabPanel("Grid", 
                       fluidRow(
                         ...
                         box(
                           title = "Grid", status = "warning", solidHeader = FALSE, width = 6, collapsible = TRUE, 
                           height = 385, 
                           checkboxInput("grid", label = "Include grid lines at major tick marks", value = FALSE),
                           conditionalPanel(condition = "input.grid==true",
                                            fluidRow(
                                              column(width = 6,
                                                     selectInput("grid.line.color", label = h5("Line color"), 
                                                                 choices = cruz.palette.color, selected = "black"),
                                                     numericInput("grid.line.width", label = h5("Line width"), 
                                                                  value = 1, min = 1, max = 6, step = 1)
                                              ),
                                              column(width = 6,
                                                     selectInput("grid.line.type", label = h5("Line type"), 
                                                                 choices = list("Solid" = 1, "Dash" = 2, "Dot" = 3,
              ...
              tabPanel("Save", 
                       fluidRow(
                         box(
                           title = "Save map", status = "warning", solidHeader = FALSE, width = 6, 
                           helpText("When saving file in RStudio window, be sure to specify '.png', '.pdf', or '.jpeg extention"), 
                           radioButtons("download.format", label = h5("Download map as"), 
