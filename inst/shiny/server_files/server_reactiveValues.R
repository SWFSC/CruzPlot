...
load_envir <- eventReactive(input$load_app_envir_file, {
  ...
  withProgress(message = "Loading saved data", value = 0.5, {
    ...
    updateSelectizeInput(session, "planned_transects_toplot", selected = input.save$planned_transects_toplot)
    updateSelectizeInput(session, "planned_transects_toplot2", selected = input.save$planned_transects_toplot2)
    updateSelectizeInput(session, "planned_transects_color", selected = input.save$planned_transects_color)
    updateSelectizeInput(session, "planned_transects_lty", selected = input.save$planned_transects_lty)
    ...
    if (isTruthy(cruz.list$das.data)) {
      ...
      if (input.save$das_sightings) {
        ...
        updateSelectizeInput(session, "das_sighting_code_1", selected = input.save$das_sighting_code_1)
        ...
        updateSelectizeInput(session, "das_sight_cruise", selected = input.save$das_sight_cruise)
      ...
      if (input.save$das_effort != 1) {
        ...
        updateSelectizeInput(session, "das_effort_det_bft_col", selected = input.save$das_effort_det_bft_col)
        ...
        updateSelectizeInput(session, "das_effort_cruise", selected = input.save$das_effort_cruise)
