              ...
              tabPanel("Non-DAS data",
                       ...
                       fluidRow(
                         box(
                           ...
                           helpText(HTML(paste("Longitudes must be in -180 to 180 range.", 
                                               "See manual for longitude and latitude column naming requirements"))), 
                           
                           fluidRow(column(12, fileInput("ndas.file", label = h5("Load non-DAS .csv file")))),
                           textOutput("cruzNonDasFile_LonLat_text"), 
                           conditionalPanel("output.cruzNonDasFile_Conditional", 
                                            radioButtons("ndas_plot_type", label = h5("Type of data"), 
                                                         choices = list("Line" = 1, "Point" = 2), selected = 1), 
                                            
                                            conditionalPanel(condition = "input.ndas_plot_type==1",
                                                             
                                                             fluidRow(
                                                               column(width = 6,
                                                                      selectInput("ndas.line.lty", label = h5("Line type"), 
                                                                                  choices = cruz.line.type,
                                                                                  selected = 1),
                                                                      numericInput("ndas.line.lwd", label = h5("Line width"), 
                                                                                   value = 1, min = 1, max = 6, step = 1)
                                                               ),
                                                               column(width = 6,
                                                                      selectInput("ndas.line.col", label = h5("Line color"), 
                                            ...
                                            conditionalPanel(condition = "input.ndas_plot_type==2",
                                                             fluidRow(
                                                               column(width = 6,
                                                                      selectInput("ndas.pt.pch", label = h5("Point type"), 
                                                                                  choices = cruz.symbol.type,
                                                                                  selected = 1),
                                                                      numericInput("ndas.pt.cex", label = h5("Point size"), 
                                                                                   value = 1, min = 0.1, max = 5, step = 0.1)
                                                               ),
                                                               column(width = 6,
                                                                      selectInput("ndas.pt.col", label = h5("Point color"), 
                                                                                  choices = cruz.palette.color,
                                                                                  selected = "black"),
                                                                      numericInput("ndas.pt.lwd", label = h5("Point line width"), 
                                                                                   value = 1, min = 1, max = 6, step = 1)
                                                               )
                                                             )
                                            ), 
                         ...
                         box(
                           ...
                           conditionalPanel("input.ndas_plot", 
